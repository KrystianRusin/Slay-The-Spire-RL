...
def train(model, deliveries, total_steps, progress=None, save_path=SAVE_PATH, publisher=None):
...
def main():
    ...
    with closing(PolicyPublisher(config)) as publisher, closing(RolloutConsumer(config)) as deliveries:
        train(model, deliveries, total_steps=100000, progress=progress, publisher=publisher)
